...
> 출처: `ai-place-srs-v1_0.md` §3.5.1 · §3.5.2 · §3.5.3
...
SRS §3.5.3의 4개 규칙이 이 모델 전체를 지배한다.
...
이 둘을 한 필드로 합치면 한도나 감면율 중 하나가 반드시 틀린다. SRS §3.2.4.1이 원장 필드 분리를 명시적으로 요구한다.
...
보존 기간 10년은 상사채권 소멸시효를 기준으로 잡았다(SRS S-2).
...
SRS L-2 · §3.1.2에 따라 **타사 계좌는 아래 필드가 비어 있다.** 세액 계산이 자사 전용인 근본 이유다.
