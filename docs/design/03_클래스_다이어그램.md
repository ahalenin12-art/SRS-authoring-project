...
> 출처: `02_SRS_연금플러스_v1.0.md` §6.6 · §6.9 · §6.4
> 이 문서는 **코드가 어떤 책임 단위로 나뉘는가**를 정의한다. ERD가 "무엇을 저장하나"라면 이것은 "무엇이 계산하나"다.
>
> 사업 동태를 다루는 **인과 순환 다이어그램(Causal Loop Diagram)** 은 별도 문서 [`08-causal-loop.md`](./08_%EC%9D%B8%EA%B3%BC%EB%A3%A8%ED%94%84_%EB%8B%A4%EC%9D%B4%EC%96%B4%EA%B7%B8%EB%9E%A8.md)에 있다.
