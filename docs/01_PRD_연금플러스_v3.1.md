...
> **개발 준비도.** 이 문서는 **Phase 0 착수가 가능한 상태**입니다. Gate는 성격이 둘로 나뉩니다.
>
> **Phase 1 착수 Gate** — 닫히지 않으면 개발에 들어가지 않습니다.
>
> | Gate | 항목 | 담당 | 미해결 시 |
> |---|---|---|---|
...
>
> **기능2 Release Gate** — 개발은 진행하되 출시를 막습니다.
>
> | Gate | 항목 | 담당 | 미해결 시 |
> |---|---|---|---|
> | **LEGAL-Q1** | 개별 계좌 세액 계산의 투자자문업 해당 여부 | 법무 | **기능2 출시 보류.** 기능1은 영향 없어 단독 출시 가능 (§15.2) |
>
> Gate가 아닌 확인 항목 13건은 미해결 상태에서도 정해진 축소형으로 인도됩니다.
...
> 4. **문서 버전 체계 정리** — 파일명을 내부 버전에 맞추고 Document ID / Version / Status / Last Updated를 분리했습니다.
...
> **v3.1 후속 정정.** 개발 인계 관점 재검토에서 나온 지적 5건을 고쳤습니다.
>
> | # | 지적 | 조치 |
> |:---:|---|---|
> | 1 | 파일명(v3_0)과 내부 버전(3.1) 불일치 | 파일명을 `ai-place-prd-v3_1.md`로 정정 |
> | 2 | 상단 Gate 3건과 §2.2 "유일한 승격 조건" 충돌 | Gate를 **Phase 1 착수**(SYS-Q3·Q7)와 **기능2 Release**(LEGAL-Q1)로 분리. §2.2 문장을 SYS-Q7 한정으로 수정 |
> | 3 | §11 "인가는 불필요하지만" — LEGAL-Q1 미확정 상태의 법적 단정 | "인가가 필요하지 않은 **구조를 전제**로 한다. 확정 전까지 기능2 미출시"로 방어적 서술 전환 |
> | 4 | 감사 로그 10년의 근거를 상법 소멸시효로 표기 (실제 §64는 **5년**) | "법무·정보보호 정책 확정값 적용. 현재 **설계 상한** 10년"으로 변경. 법정 보존기간 확정을 §15.2 Q5에 포함 |
> | 5 | P-5만 `p95` 표기 누락 | `p95 ≤ 300ms`로 통일 |
>
> **여기서 PRD를 닫습니다.** Data Contract · RTO/RPO · RTM · 상세 오류코드 · Test Case Mapping은 SRS(`03_SRS_연금플러스_ISO29148_v0.1.md`) 소관입니다. 더 붙이면 PRD와 SRS의 경계가 무너집니다.
...
**기준선 확정 절차** — 콜센터 문의 코드에 "이관 상태 확인" 분류가 없어 북극성과 자동취소율은 현재 관측되지 않습니다. Phase 0에서 문의코드 `TRF-STATUS`를 신설하고 **4주 관측치를 기준선으로 확정**합니다. SYS-Q7은 **핵심 지표 측정 관점의 Phase 0 승격 필수 조건**입니다. 전체 Phase 1 착수 Gate는 §15를 따릅니다.
...
| P-5 | 인출액 변경 시 재계산 | p95 ≤ 300ms | `f2_recalc_ms` | p95가 500ms 초과 |
...
| S-2 | 감사 로그 보존 기간 | **법무·정보보호 정책 확정값 적용. 현재 설계 상한 10년** | `audit_retention_days` | 보존 정책 위반 감지 시 |
...
현재 설계는 **별도 투자자문업 인가가 필요하지 않은 구조를 전제**로 합니다. 다만 이 전제 자체가 LEGAL-Q1(§15.2)로 미확정 상태이므로, **확정 전까지 기능2는 출시하지 않습니다.** 아래 조건이 변경되면 규제 지위를 재검토합니다. 인용 조문은 원문으로 확인했습니다.
...
| 5 | 예약 전송 버튼을 **전자적 의사표시**로 볼 때, 인증수단과 로그 보존 범위는 어디까지 필요합니까? 감사 로그·개인정보의 **법정 보존기간**은 몇 년입니까? | F1-03 전송 구조 · 보존 정책 | 축소 | 설계 상한 10년으로 운영하되 정책 확정 시 즉시 조정 |
...
| 문서 | 역할 |
|---|---|
| 이 문서 (`ai-place-prd-v3_1`) | 기능1·2 통합 PRD |
