...
| Version | 2.0 |
| 작성일 | 2026-08-25 |
| 대상 문서 | [04 SRS 기술제약반영 v1.0](./04_SRS_%EC%97%B0%EA%B8%88%ED%94%8C%EB%9F%AC%EC%8A%A4_%EA%B8%B0%EC%88%A0%EC%A0%9C%EC%95%BD%EB%B0%98%EC%98%81_v1.0.md) |
| 선행 버전 | [05 태스크 리스트 전체판](./05_%5B%ED%83%9C%EC%8A%A4%ED%81%AC%20%EB%A6%AC%EC%8A%A4%ED%8A%B8%5D%20%EC%97%B0%EA%B8%88%ED%94%8C%EB%9F%AC%EC%8A%A4_%EC%A0%84%EC%B2%B4%ED%8C%90.md) — 전체 102개판 |
| 총 태스크 | **72** = 병합 결과 66 (개발 49 / 디자인 17) + 감사 보정 신규 6 (FR-050 · FR-051~055) |
| 감축 | 102 → 66 (−36) · 이후 감사 보정으로 +6 |
...
[[분석] 태스크 추출 방법론 적합성 평가](./07_%5B%EB%B6%84%EC%84%9D%5D%20%ED%83%9C%EC%8A%A4%ED%81%AC%20%EC%B6%94%EC%B6%9C%20%EB%B0%A9%EB%B2%95%EB%A1%A0%20%EC%A0%81%ED%95%A9%EC%84%B1%20%ED%8F%89%EA%B0%80%20%28%EC%97%B0%EA%B8%88%ED%94%8C%EB%9F%AC%EC%8A%A4%29.md)에서 발견한 결함 **D-01(성능 SLO 실측 태스크 누락)** 의 보정이다. **v1에서 병합되어 온 것이 아니라 새로 추가된 1건**이므로 위 표와 구분해 기록한다.

| Task ID | Epic (도메인) | Feature (기능명) | 관련 SRS 섹션 | 선행 태스크 | 복잡도 | 병합 원본 |
|---|---|---|---|---|:---:|---|
| FR-050 | Quality Gate | 성능 SLO 실측 — p95 5종 부하 시험 및 콜드 스타트 영향 측정 | 9.1 CONFLICT-01 / OPEN-TEC-003 | FR-004, FR-027 | M | **신규 (D-01 보정)** |
| FR-051 | Verification | GWT 인수 기준 76건 ↔ 시험 케이스 추적 매트릭스 | PRD §5 (AC 표 12개) | FR-016 | **H** | **신규 (D-02 보정)** |
| FR-052 | Verification | 상태 전이 통합 시험 — 허용 16종 + 금지 5종 | PRD §8.6 전이 매트릭스 | FR-013, FR-022 | **H** | **신규 (D-02 보정)** |
| FR-053 | Verification | 예외·멱등·정합성 시험 — E-01~12 · ID-1~5 · RC-1~5 | PRD §8.7 | FR-020, FR-028, FR-032 | **H** | **신규 (D-02 보정)** |
| FR-054 | Verification | E2E 고객 여정 시험 — 사전조회 → 신청 → 상태확인 → 완료 | PRD §5 사용자 스토리 | FR-040, FR-041, FR-042 | M | **신규 (D-02 보정)** |
| FR-055 | Verification | 접근성 자동 검사 파이프라인 편입 | 6.3 TEC-UI-010~012 | FR-037 | M | **신규 (D-02 보정)** |
...
| H (건드리지 않음) | 30 | **30** | 0 |
| M | 52 | 34 | −18 |
| 감사 보정 신규 (FR-050 · FR-051~055) | — | +6 | +6 |
| **실행 기준 총계** | 102 | **72** | **−30** |
| L | 20 | 2 | −18 |
