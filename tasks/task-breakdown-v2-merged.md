...
| 총 태스크 | **67** = 병합 결과 66 (개발 49 / 디자인 17) + 감사 보정 신규 1 (FR-050) |
| 감축 | 102 → 66 (−36) · 이후 감사 보정으로 +1 |
...
[[분석] 태스크 추출 방법론 적합성 평가](../docs/%5B%EB%B6%84%EC%84%9D%5D%20%ED%83%9C%EC%8A%A4%ED%81%AC%20%EC%B6%94%EC%B6%9C%20%EB%B0%A9%EB%B2%95%EB%A1%A0%20%EC%A0%81%ED%95%A9%EC%84%B1%20%ED%8F%89%EA%B0%80%20%28%EC%97%B0%EA%B8%88%ED%94%8C%EB%9F%AC%EC%8A%A4%29.md)에서 발견한 결함 **D-01(성능 SLO 실측 태스크 누락)** 의 보정이다. **v1에서 병합되어 온 것이 아니라 새로 추가된 1건**이므로 위 표와 구분해 기록한다.

| Task ID | Epic (도메인) | Feature (기능명) | 관련 SRS 섹션 | 선행 태스크 | 복잡도 | 병합 원본 |
|---|---|---|---|---|:---:|---|
| FR-050 | Quality Gate | 성능 SLO 실측 — p95 5종 부하 시험 및 콜드 스타트 영향 측정 | 9.1 CONFLICT-01 / OPEN-TEC-003 | FR-004, FR-027 | M | **신규 (D-01 보정)** |

> §9.1이 완화책 3종(Edge Runtime · TTL 캐시 · 워밍)을 제시한 뒤 "완화 후에도 p95 500ms 보장이 확정되지 않는다. **Phase 0에서 실측 후 판정**"이라고 명시했다. v1·v2 모두 완화책 구현(FR-004)만 있고 실측이 없어, **완화 여부를 판정할 수단 없이 출시 게이트에 도달**하는 구조였다.
