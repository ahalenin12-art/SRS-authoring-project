...
**배포 단위와 외부 경계** — 상세는 [설계 04 컴포넌트 다이어그램](design/04-component-diagram.md)
...
전체 필드 정의와 마이데이터 한계는 [설계 02 데이터 모델](design/02-erd.md)에 있다.
...
전체 11개 시퀀스 전문은 [설계 05 시퀀스 다이어그램](design/05-sequence-diagram.md)에 있다. 여기에는 **실패 경로가 있는 3개**만 싣는다.
...
> **호출 순서를 확인하려면** [설계 05 시퀀스 다이어그램](design/05-sequence-diagram.md)을 함께 본다. 시퀀스 SD-01 ~ SD-10이 아래 인수 조건과 1:1로 대응한다.
>
> | 시퀀스 | 대응 인수 조건 |
> |---|---|
...
**산출 흐름** — 상세는 [설계 07 FC-01](design/07-flowchart.md#fc-01-연금수령한도-산출)
...
**차감 흐름** — 상세는 [설계 07 FC-02](design/07-flowchart.md#fc-02-인출순서-3층-차감)
...
**폴백 사다리** — 위에서부터 조건이 맞는 첫 단계를 사용한다. 상세는 [설계 07 FC-06](design/07-flowchart.md#fc-06-완료일-밴드-폴백-사다리)
...
**상태 전이** — 상세는 [설계 06 상태 다이어그램](design/06-state-diagram.md)
...
> 원본 프로토타입(`통합_프로토타입_최종_V7.html`)과 그 내용 전문(`통합_프로토타입_최종_V7.md`)에 작성된 내용이다. §6.7.3 검증 데이터셋의 출처이므로 검증 기준의 신뢰도에 직결된다.
...
| 문서 | 역할 |
|---|---|
| [`design/`](design/README.md) | **이 SRS에서 파생된 기술 설계 문서 8종.** 유스케이스·ERD·클래스·컴포넌트·시퀀스·상태·흐름도·인과 순환 (다이어그램 45개) |
