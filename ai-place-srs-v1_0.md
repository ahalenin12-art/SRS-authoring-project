...
> **양식에 관한 고지.** 예시 SRS(`[SRS 문서] AD-Core-Platform (한글).md`)의 목차 중 **§3 시스템 컨텍스트 및 인터페이스 · §4 세부 요구사항 · §6 부록**을 확보해 그 구성으로 재배치했습니다.
>
> | 절 | 예시 양식 | 이 문서의 대응 |
> |---|---|---|
> | 3.1 | 클라이언트 애플리케이션 | 화면 12종 · 전이도 |
> | 3.2 | 외부 시스템 연동 | 인터페이스 I-1 ~ I-9 · 통신 규약 |
> | 3.3 | API 개요 | 네 갈래 · 설계 원칙 4건 · 응답 코드 |
> | 3.4 | 인터랙션 시퀀스 (핵심·간결) | 이체 예약·전송 / 진행 단계 갱신 / 인출순서·세액 / 매매창 판정 |
> | 6.1 | API 엔드포인트 목록 | 이관 9건 · 인출 7건 · 내부 4건 |
> | 6.2 | 엔티티 및 데이터 모델 | 원장 필드 15종 · 엔터티 7종 · 마이데이터 한계 |
> | 6.3 | 상세 인터랙션 모델 (확장) | 실패 경로가 있는 3개 시퀀스 |
>
> **§3.4의 하위 항목은 대체했습니다.** 예시의 3.4.1~3.4.4(문서 자동 생성 · Validator 실행 · PMF 진단 · Notion/Jira 동기화)는 AD-Core-Platform의 기능이라 이 제품에 대응 항목이 없습니다. 같은 자리에 연금플러스의 핵심 흐름 4개를 넣었습니다.
>
> **§1 · §2 · §5는 예시 목차를 확보하지 못했습니다.** ISO/IEC/IEEE 29148:2018 Annex C 구성을 유지하고 있으며, 해당 부분 목차를 받으면 그 양식으로 다시 맞춥니다.
>
> **[추가 챕터] 표시의 의미.** 원본 문서(PRD·프로토타입)에 이미 작성되어 있으나 예시 양식에 대응 절이 없는 내용입니다. 새로 만들어 넣은 것이 아니라 **기존 작성분을 표준 구성에 따라 배치한 것**입니다.
>
> | 절 | 내용 | 출처 | 표준상 근거 |
> |---|---|---|---|
> | §4.3 | 법정 계산 규칙 | PRD §7 | 산식이 화면 출력을 직접 결정하므로 기능 요구사항의 일부 |
> | §4.7.1 | 규제 준수 제약 | PRD §11 | Annex C 3.6 설계 제약사항 |
> | §6.5 | 설계 결정 기록 (ADR) | PRD §13.2 | 요구사항 근거 기록 (§5.2.8) |
> | §6.6 | 리스크 등록부 | PRD §14 | Annex C 3.8 지원 정보 |
> | §6.7 | 단계적 인도 계획 | PRD §12 | 요구사항 인도 시점 규정 |
> | §6.10 | 사업 배경 및 시장 근거 | PRD §1.1·1.2·2.1·2.3 | 이해관계자 요구사항(StRS) 영역 |
> | §6.11 | AS-IS 갭 분석과 요구사항 대응 | PRD §1.3·1.4 | 요구사항 출처 추적 (§5.2.6) |
> | §6.12 | 사용자 프로파일 상세 | PRD §4 | §1.3.3 사용자 특성의 확장 |
> | §6.13 | 벤치마크 분석 | PRD §16.2 | 설계 참조 근거 |
> | §6.14 | 프로토타입 검증 기준 | 프로토타입 V7 | §5.3 검증 데이터셋의 출처 |
...
| 로보어드바이저 단독 연결·수익률 약속형 문구 | 중립성 훼손 (§4.7.1) | — |
...
**설계 전제 — ③④ 구간에는 실시간 조회 경로가 존재하지 않는다.** 이관사 의사확인(③)과 자산 현금화(④)는 이관사 내부 처리다. 수관사가 받을 수 있는 것은 전문 수신 시점뿐이다. 이 사실 위에 추정 레이어(§4.2)와 밴드 표시(§4.3.5)를 설계했다.
...
**누가 무엇을 하는가** — 상세는 [설계 01 유스케이스 명세](design/01-usecase.md)
...
단, 기능1의 F1-06이 확정한 **적용 가입일 값은 기능2의 한도 산식 입력으로 전달된다** (§5.2 AC F1-06 #5).
...
| L-6 | 화면이 종목·수량·시기를 지정하면 투자자문업 정의에 해당한다 | 법규 | §4.7.1의 11개 제약 준수 |
...
| WCAG 2.1 Level AA | 색 대비 요구사항 (§4.4) |
...
|:---:|---|:---:|---|:---:|---|
| I-1 | KB증권 연금 원장 | 조회 | 계좌 개설일·승계 플래그·재원별 잔액·연간 누적 인출액 등 (§4.6) | 확정 | — |
...
핵심 흐름 4개의 간결한 버전이다. 오류 분기와 대안 흐름까지 담은 확장 버전은 §6.3에 있다.
...
**연차 필드 분리 요구** — 한도용 연차는 최초로 연금수령할 수 있는 날이 속하는 과세기간부터 실제 수령 여부와 무관하게 누적된다. 감면율용 연차는 실제로 수령한 연차만 카운트한다. **원장에서 두 필드를 분리 관리해야 한다** (§4.6).
...
원장에서 확보해야 할 필드 15종과 핵심 엔터티 7종의 정의는 **§6.2 엔티티 및 데이터 모델**에 있다. 이 절에는 그 위에 걸리는 **설계 규칙**만 둔다.
...
| M-3 | 릴리스마다 §5.3 검증 데이터셋 전건을 대조한다 | 6건 전건 통과 |
...
| 8 | 검증 계좌 조건이다 (§5.3) | 한도를 계산한다 | 한도 12,600,000원, 남은 금액 10,600,000원이 산출되고 응답 p95 ≤ 1초 | **실패** — 검증값과 1원이라도 다르면 |
...
| 3 | 검증 계좌 조건이다 (§5.3) | 15,000,000원 인출을 계산한다 | 일반 465,960원, 확인서 제출 후 355,080원, 부득이한 사유 378,840원 | **실패** — 세 값 중 하나라도 다르면 |
...
|---|:---:|---|---|---|
| FR-F1-01-* | 5 | §5.2 F1-01 (6건) | P-1, C-1 | — |
| FR-F1-02-* | 5 | §5.2 F1-02 (6건) | U-1 ~ U-4 | — |
| FR-F1-03-* | 11 | §5.2 F1-03 (11건) | P-2, P-6, S-1, A-5 | 밴드 검증값 |
| FR-F1-04-* | 12 | §5.2 F1-04 (10건) | P-2, P-3, U-1, U-2 | — |
| FR-F1-05-* | 5 | §5.2 F1-05 (5건) | M-4 | 밴드 검증값 |
| FR-F1-06-* | 6 | §5.2 F1-06 (6건) | R-8, C-1 | 밴드 검증값 |
| FR-F2-01-* | 3 | §5.2 F2-01 (4건) | P-4 | — |
| FR-F2-02-* | 3 | §5.2 F2-02 (3건) | — | — |
| FR-F2-03-* | 12 | §5.2 F2-03 (9건) | P-4, P-5, R-7, R-11 | 계좌 검증값 1~3 |
| FR-F2-04-* | 12 | §5.2 F2-04 (8건) | P-5, A-4, R-1, R-3, R-4, M-1 | 계좌 검증값 4~6 |
| FR-F2-05-* | 5 | §5.2 F2-05 (4건) | R-6, 중립성 ① | — |
| FR-F2-06-* | 4 | §5.2 F2-06 (4건) | R-3 (적용 제외) | — |
| **합계** | **83** | **76건** | — | **8건** |
...
| T-8 | DELETE | `/transfers/{id}` | 예약 폐기 또는 신청 취소 (당일·의사확인 전) | 본인 인증 | §4.2 취소 구간 |
...
§4.6의 설계 규칙(D-1 ~ D-4)이 적용된 실제 필드 정의다. 논리 모델이며 물리 스키마가 아니다. 엔터티 관계도 전문은 [설계 02 데이터 모델](design/02-erd.md)에 있다.

#### 6.2.1 원장에서 확보해야 할 필드
...
**엔터티 관계** — 전체 ERD와 필드 정의는 [설계 02 데이터 모델](design/02-erd.md)
...
> **`LOCK_WINDOW`에 단일 날짜 컬럼이 없다.** `end_band_from` / `end_band_to` 두 컬럼만 둔다. 단일 날짜로 저장하면 그 값을 확정처럼 보여주게 되고 금소법 §21①의 단정 금지에 걸린다. **저장 구조 자체가 단정을 막는다.**
>
> **`ACCOUNT`의 연차 필드가 두 개인 이유.** `pension_start_applied_at`(한도용, 실제 수령 여부 무관 누적)과 `first_receipt_at`(감면율용, 실제 수령 연차만)을 합치면 한도나 감면율 중 하나가 반드시 틀린다.
...
#### 6.2.3 마이데이터로 채울 수 없는 필드

L-2에 따라 **타사 계좌는 아래 필드가 비어 있다.** 세액 계산이 자사 전용인 근본 이유다.

| 필드군 | 마이데이터 제공 |
|---|:---:|
| 계좌 평가액 · 개설일 · 최초납입일 · 거래내역 | **제공** |
| 과세제외금액 (1층 4버킷) | ✕ |
| 이연퇴직소득 잔액 · 적용 퇴직소득세율 | ✕ |
| 세액공제 받은 납입액 (3층) | ✕ |
...
따라서 **한도는 타사 계좌까지 계산 가능**(평가액·개설일만 필요), **세액은 자사 계좌 전용**이다.

---

### 6.3 상세 인터랙션 모델

§3.4의 간결 버전에서 생략한 **오류 분기와 대안 흐름**을 담은 확장 버전이다. 각 시퀀스는 §5.2 인수 조건과 1:1로 대응한다.

전체 11개 시퀀스 전문은 [설계 05 시퀀스 다이어그램](design/05-sequence-diagram.md)에 있다. 여기에는 **실패 경로가 있는 3개**만 싣는다.

#### 6.3.1 전송 확장 — 감사 로그 실패 시 롤백

§3.4.1의 확장. 기록 없는 의사표시는 허용하지 않는다.
...
**대응 인수 조건** — §5.2 F1-03 #4, #8, #10

#### 6.3.2 밴드 산출 확장 — 마이데이터 타임아웃과 폴백

§3.4.1의 예약 저장 단계 확장. 빈 화면이나 무한 로딩은 실패다.
...
**대응 인수 조건** — §5.2 F1-03 #1, #7, #11 · F1-05 #3, #4

#### 6.3.3 시뮬레이션 확장 — 계산 전 두 가드

§3.4.3의 확장. 가드는 **계산 전에** 걸린다. 후에 걸면 이미 틀린 값이 만들어진 뒤다.
...
**대응 인수 조건** — §5.2 F2-03 #4, #5, #8 · F2-04 #3, #6
...
| **ADR-007** | 밴드 산출에 평균이 아니라 백분위(p50~p80)를 쓴다. 분쟁·압류 건이 분포의 꼬리를 길게 만든다 | §4.3.5 |
...
착수 전 확인이 필요한 항목이다. **결정을 미루는 목록이 아니다.** 각 항목은 §6.6에 대응책이 걸려 있어 답이 늦어져도 정해진 축소형으로 인도된다.
...
| 4 | 계산 오류 시 면책 문구로 어디까지 방어되는가? | 축소 | §4.7.1의 실효적 방어 4종을 전부 적용 |
...
#### 6.10.2 차별화 축 — 어디가 비어 있는가

| 구간 | 현재 시장 | 판정 | 대응 |
|---|---|---|---|
| **신청 전** 판별 | 예탁결제원이 실물이전 가능 여부 사전조회를 무료 제공 (2025.7.21~) | 기본기. 여기서는 차별화되지 않음 | 범위 제외 (§1.2.3) |
...
### 6.11 AS-IS 갭 분석과 요구사항 대응 **[추가 챕터]**

> 원본 PRD §1.3 · §1.4에 작성된 내용이다. ISO/IEC/IEEE 29148 §5.2.6은 요구사항이 그 출처까지 추적 가능할 것을 요구한다. 이 부록은 각 요구사항이 **어느 실측 갭에서 나왔는지**를 연결한다.
...
| 갭 ID | 대응 요구사항 | 검증 |
|:---:|---|---|
| G-1 | FR-F1-02-01 ~ 05 | §5.2 F1-02 #1 ~ #6 |
| G-2 | FR-F1-01-01 · FR-F1-04-01 ~ 04 | §5.2 F1-01 #2, F1-04 #1 ~ #4, #10 |
| G-3 | FR-F1-03-07 · FR-F1-04-09 | §5.2 F1-03 #3, #4, F1-04 #8 |
| G-4 | FR-F1-03-04 | §5.2 F1-03 #3, #4 |
| G-5 | FR-F1-03-01 · 03 · 06 | §5.2 F1-03 #5 ~ #7 |
| G-6 | FR-F2-01-01 · FR-F2-02-01 | §5.2 F2-01 #1, F2-02 #1 |
| G-7 | FR-F2-04-01 · 09 | §5.2 F2-04 #1, #3 |
| G-8 | FR-F2-05-04 · FR-F2-06-01 | §5.2 F2-05 #2, F2-06 #1 |

**정상 구간** — 신청 플로우 ①~④·⑥은 갭이 확인되지 않았다. 이 구간을 변경하는 요구사항은 두지 않는다.

### 6.12 사용자 프로파일 상세 **[추가 챕터]**

> 원본 PRD §4에 작성된 내용이다. §1.3.3의 사용자 특성 표를 확장한 것으로, 각 요구사항이 누구의 어떤 문제를 푸는지 연결한다.

#### 6.12.1 서동현 (41세, 적극투자형, PB 미배정) · 기능1
...
#### 6.12.3 최은호 (상속·승계) · 기능2

> 승계라는 절차의 존재만 알게 되고 처리 방법이 안내되지 않아, 무엇부터 할지 정하지 못한다.

**대응** — FR-F2-06-01 ~ 04. 상속인 조회는 계좌를 **찾기 이전** 단계이고, 찾은 뒤 서류를 준비하는 것은 F10 소관이다 (§1.2.3).
...
**대응** — FR-F1-04-07 (평이화된 사유와 해소 절차 3단계). 개별 해소 대행은 F4 소관이다 (§1.2.4).
...
### 6.14 프로토타입 검증 기준 **[추가 챕터]**

> 원본 프로토타입(`통합_프로토타입_최종_V7.html`)과 그 내용 전문(`통합_프로토타입_최종_V7.md`)에 작성된 내용이다. §5.3 검증 데이터셋의 출처이므로 검증 기준의 신뢰도에 직결된다.
...
#### 6.14.2 검증 기준값 불일치 — **해소 필요**

§5.3의 밴드 검증값은 PRD에서 가져왔으나, **실제 V7 프로토타입의 값과 다르다.** QA가 프로토타입을 기준으로 검증하면 전건 실패한다.

| 항목 | PRD (§5.3 채택값) | V7 프로토타입 실측 | 차이 |
|---|---:|---:|---:|
| 보유 6종목 평가금액 합계 | 48,500,000원 | 44,890,000원 | **3,610,000원** |
| 병목 정리 시 확정 손실 | 420,000원 | 312,000원 | **108,000원** |
| 당겨지는 영업일 수 | 3영업일 | 3영업일 | 일치 |

> **조치** — 착수 전에 어느 쪽을 검증 기준으로 삼을지 확정한다. 확정 전까지 §5.3의 **밴드 검증값은 인수 판정에 사용하지 않는다.** 계좌 검증값 6건(세액 계산)은 이 불일치와 무관하므로 그대로 사용한다.

#### 6.14.3 프로토타입 자체의 내부 불일치
...
> **조치** — 4번과 5번은 요구사항(FR-F2-01-01, FR-F1-03-11)에 이미 정의되어 있으므로 구현 시 SRS를 따른다. 1번과 6번은 §5.3 검증값 확정과 함께 정리한다.

---

## 면책

이 문서는 제품 구현을 위한 요구사항 명세이며 **법률·세무 자문이 아니다.** 조문 인용은 국가법령정보센터·국세청 공개 법령 자료 원문으로 확인했다. §4.7.1의 규제 해당 여부는 이 문서가 내린 설계 판단이며, 출시 전 법무·컴플라이언스 검토로 확정한다 (§6.8).
