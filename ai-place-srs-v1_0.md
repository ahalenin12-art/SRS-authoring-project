...
>
> **[추가 챕터] 표시의 의미.** 원본 문서(PRD·프로토타입)에 이미 작성되어 있으나 일반적인 SRS 본문 구성에 대응 절이 없는 내용입니다. 새로 만들어 넣은 것이 아니라 **기존 작성분을 ISO/IEC/IEEE 29148 구성에 따라 배치한 것**입니다.
>
> | 절 | 내용 | 출처 | 표준상 근거 |
> |---|---|---|---|
> | §3.2.4 | 법정 계산 규칙 | PRD §7 | 산식이 화면 출력을 직접 결정하므로 기능 요구사항의 일부 |
> | §3.6.1 | 규제 준수 제약 | PRD §11 | Annex C 3.6 설계 제약사항 |
> | 부록 B | 설계 결정 기록 (ADR) | PRD §13.2 | 요구사항 근거 기록 (§5.2.8) |
> | 부록 C | 리스크 등록부 | PRD §14 | Annex C 3.8 지원 정보 |
> | 부록 D | 단계적 인도 계획 | PRD §12 | 요구사항 인도 시점 규정 |
> | 부록 G | 사업 배경 및 시장 근거 | PRD §1.1·1.2·2.1·2.3 | 이해관계자 요구사항(StRS) 영역 |
> | 부록 H | AS-IS 갭 분석과 요구사항 대응 | PRD §1.3·1.4 | 요구사항 출처 추적 (§5.2.6) |
> | 부록 I | 사용자 프로파일 상세 | PRD §4 | §1.3.3 사용자 특성의 확장 |
> | 부록 J | 벤치마크 분석 | PRD §16.2 | 설계 참조 근거 |
> | 부록 K | 프로토타입 검증 기준 | 프로토타입 V7 | §4.3 검증 데이터셋의 출처 |
...
### 부록 H. AS-IS 갭 분석과 요구사항 대응 **[추가 챕터]**
...
| 갭 ID | 대응 요구사항 | 검증 |
|:---:|---|---|
| G-1 | FR-F1-02-01 ~ 05 | §4.2 F1-02 #1 ~ #6 |
| G-2 | FR-F1-01-01 · FR-F1-04-01 ~ 04 | §4.2 F1-01 #2, F1-04 #1 ~ #4, #10 |
| G-3 | FR-F1-03-07 · FR-F1-04-09 | §4.2 F1-03 #3, #4, F1-04 #8 |
| G-4 | FR-F1-03-04 | §4.2 F1-03 #3, #4 |
| G-5 | FR-F1-03-01 · 03 · 06 | §4.2 F1-03 #5 ~ #7 |
| G-6 | FR-F2-01-01 · FR-F2-02-01 | §4.2 F2-01 #1, F2-02 #1 |
| G-7 | FR-F2-04-01 · 09 | §4.2 F2-04 #1, #3 |
| G-8 | FR-F2-05-04 · FR-F2-06-01 | §4.2 F2-05 #2, F2-06 #1 |

**정상 구간** — 신청 플로우 ①~④·⑥은 갭이 확인되지 않았다. 이 구간을 변경하는 요구사항은 두지 않는다.

### 부록 I. 사용자 프로파일 상세 **[추가 챕터]**

> 원본 PRD §4에 작성된 내용이다. §1.3.3의 사용자 특성 표를 확장한 것으로, 각 요구사항이 누구의 어떤 문제를 푸는지 연결한다.

#### I.1 서동현 (41세, 적극투자형, PB 미배정) · 기능1
...
### 부록 K. 프로토타입 검증 기준 **[추가 챕터]**

> 원본 프로토타입(`통합_프로토타입_최종_V7.html`)과 그 내용 전문(`통합_프로토타입_최종_V7.md`)에 작성된 내용이다. §4.3 검증 데이터셋의 출처이므로 검증 기준의 신뢰도에 직결된다.
...
#### K.2 검증 기준값 불일치 — **해소 필요**

§4.3의 밴드 검증값은 PRD에서 가져왔으나, **실제 V7 프로토타입의 값과 다르다.** QA가 프로토타입을 기준으로 검증하면 전건 실패한다.

| 항목 | PRD (§4.3 채택값) | V7 프로토타입 실측 | 차이 |
|---|---:|---:|---:|
| 보유 6종목 평가금액 합계 | 48,500,000원 | 44,890,000원 | **3,610,000원** |
| 병목 정리 시 확정 손실 | 420,000원 | 312,000원 | **108,000원** |
| 당겨지는 영업일 수 | 3영업일 | 3영업일 | 일치 |

> **조치** — 착수 전에 어느 쪽을 검증 기준으로 삼을지 확정한다. 확정 전까지 §4.3의 **밴드 검증값은 인수 판정에 사용하지 않는다.** 계좌 검증값 6건(세액 계산)은 이 불일치와 무관하므로 그대로 사용한다.

#### K.3 프로토타입 자체의 내부 불일치
...
> **조치** — 4번과 5번은 요구사항(FR-F2-01-01, FR-F1-03-11)에 이미 정의되어 있으므로 구현 시 SRS를 따른다. 1번과 6번은 §4.3 검증값 확정과 함께 정리한다.
